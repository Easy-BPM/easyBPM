...
class AmqpConfig {
    ...
    @Bean
    fun bindingRequests(queue: Queue, exchange: TopicExchange): Binding =
        BindingBuilder.bind(queue).to(exchange).with(REQUEST_ROUTING_KEY)

    @Bean
    fun bindingCompletions(queue2: Queue, exchange: TopicExchange): Binding =
        BindingBuilder.bind(queue2).to(exchange).with(COMPLETION_ROUTING_KEY)
